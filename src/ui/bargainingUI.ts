...
export function renderBargainingPanel(state: BargainingPanelState): string {
  ...
  return `
    <form id="bargaining-form" class="bargaining-form">
      ...
      <div class="stat-row"><span>Relationship</span><strong>${selectedFaction.relationship}</strong></div>
    ...
    <form id="bargain-chat-form" class="bargain-chat-form">
      <label>
        Natural Language Bargain
        <textarea id="bargain-message-input" rows="3" placeholder="Example: I offer Nova 500 credits for 10 fuel as humanitarian support."></textarea>
      </label>
      <button class="command-button bargain-submit" type="submit">Send Message</button>
    </form>
...
export function bindBargainingControls(container: HTMLElement, handlers: BargainingHandlers): void {
  ...
  container.querySelector<HTMLFormElement>('#bargain-chat-form')?.addEventListener('submit', (event) => {
    event.preventDefault();
    const input = container.querySelector<HTMLTextAreaElement>('#bargain-message-input');
